...
SLH-DSA-SHA2-128s address: slhdsaSha2S128`;
  ...
  // some of these tests require fresh versions of `.._attestation_doc.b64` and `.._ml_kem_768_ciphertext.b64`,
  // because attestation docs expire after a few hours
  ...
  describe('Attestation document verification', () => {
    ...
    //   // Should not throw
    //   await expect(verifyAttestationDoc(
    //     attestationDoc,
    ...
    // note: these tests require a fresh prod attestation doc and ciphertext
    //
    // test('validates prod attestation document with correct PCR8', async () => {
    //   const attestationDoc = (await fs.promises.readFile(
    //     'src/__tests__/test_data/production_attestation_doc.b64',
    //     'utf8'
    //   )).trim() as AttestationDocBase64;

    //   // Read and decode the ML-KEM-768 ciphertext from file
    //   const encodedCiphertext = (await fs.promises.readFile(
    //     'src/__tests__/test_data/production_ml_kem_768_ciphertext.b64',
    //     'utf8'
    //   )).trim();
    //   const ciphertext = base64.decode(encodedCiphertext) as MlKem768CiphertextBytes;
